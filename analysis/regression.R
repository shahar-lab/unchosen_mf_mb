...
df=df%>%mutate(reoffer_ch_person=(lag(ch_person)==person1)|(lag(ch_person)==person2),
               ...
               current_common_previous_reward=factor(if_else(common_object==lag(common_object),
               ...
               previous_unique_unch_exp_val=lag(expval_unique_unch)-0.5,
...
mb_common_b =
  brm(
    formula=stay_common_object~1+previous_common_reward*previous_common_exp_val+(previous_common_reward*previous_common_exp_val|subject),
...
mb_unch_modulation_b =
  brm(
    formula=ch_prev_unch_unique_object~previous_unique_reward*c_mb_unch+(previous_unique_reward||subject),
    data = df%>%filter(reoffer_unch_person==F,reoffer_ch_person==F,reoffer_unique_unch_object==T),
    family = bernoulli(link = "logit"),
    warmup = 1000,
    iter = 2000,
