...
for (block in 1:Nblocks){
  ...
  for (trial in 1:Ntrials_perblock){
    # computer offer function
    shares_product = list(c(1,2),c(2,3),c(3,4),c(4,5),c(1,5)) #only pairs who share an product are offered
   
    if (!is.null(last_pair)) {
      shares_product <- shares_product[!sapply(shares_product, function(x) all(sort(x) == sort(last_pair)))] #remove currently offered pair
    }
    raffle <- sample(sample(shares_product, 1)[[1]])
    # update last_pair
    last_pair <- raffle
    #players choice
